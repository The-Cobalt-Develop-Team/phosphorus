...
int main(int argc, char *argv[]) {
  ...
  SCI_CONST PI = 3.14159265358979323846; // Pi constant

  // Equation: G * M / r^2 =  v0^2 / r
  // => v0 = sqrt(G * M / r)
  double R = AU; // Distance from the Sun in meters
  if (argc > 1) {
    R = std::stod(argv[1]) * AU; // Allow user to specify distance
  }
  double v0 = std::sqrt(G * M / R); // Initial velocity in m/s

  // Equation: v0 = R * omega, T = 2 * pi / omega
  double T = 2 * PI * R / v0; // Orbital period in seconds

  // The gravity field is implemented in Field.h
  Cartesian2DGravityField field({0, 0}, M);
  Cartesian2D initial_position{R, 0};
  ...
  animator.generate("EarthOrbit", result, 60);
  cout << "Animation generated: EarthOrbit\n";
